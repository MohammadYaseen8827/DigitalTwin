...
//             // Only seed if no machines exist (first run)
//             var hasMachines = await _context.Machines.AnyAsync();
//             if (hasMachines)
//             {
//                 _logger.LogInformation("Database already has machines. Skipping seed data initialization.");
//                 return;
//             }
...
//         _logger.LogInformation("Added {Count} new machines (attempted {Total}).", machinesAdded, machines.Count);
//     }
...
//         // Check if production lines already exist by name to avoid duplicates
//         foreach (var name in productionLines)
//         {
//             var existing = await _context.ProductionLines.FirstOrDefaultAsync(pl => pl.Name == name);
...
//                 {
//                     // Let EF Core generate the ID automatically
//                     Name = name,
...
//         await _context.SaveChangesAsync();
//         _logger.LogInformation("Ensured {Count} production lines exist.", productionLines.Count);
//     }
...
//         {
//             // Check for existing alert based on unique combination of Message, MachineId and creation time
//             var existing = await _context.Alerts.FirstOrDefaultAsync(a => 
//                 a.Message == alert.Message && 
//                 a.MachineId == alert.MachineId && 
//                 Math.Abs((a.CreatedAt - alert.CreatedAt).TotalMinutes) < 5); // Within 5 minutes to handle timing differences
//             if (existing == null)
//             {
...
//         await _context.SaveChangesAsync();
//         _logger.LogInformation("Added {Count} new alerts (attempted {Total}).", alertsAdded, alerts.Count);
//     }
...
//         var simulationsAdded = 0;
//         foreach (var simulation in simulations)
//         {
//             // Check for existing simulation based on unique combination of MachineId and time period
//             var existing = await _context.SimulationStates.FirstOrDefaultAsync(s => 
//                 s.MachineId == simulation.MachineId && 
//                 s.StartTime == simulation.StartTime && 
//                 s.EndTime == simulation.EndTime);
//             if (existing == null)
//             {
...
//         await _context.SaveChangesAsync();
//         _logger.LogInformation("Added {Count} new simulations (attempted {Total}).", simulationsAdded, simulations.Count);
//     }
// }
