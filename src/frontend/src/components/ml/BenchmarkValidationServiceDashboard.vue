...
import { useBenchmarkValidation } from '@/composables/useBenchmarkValidation'
import { 
  Target, 
  Play, 
  FileText, 
...
const handleValidate = async () => {
  ...
  try {
    ...
    if (result) {
      ...
      toast.error('Validation returned no result')
    }
...
const handleCompare = async () => {
  ...
  try {
    ...
    if (comparison) {
      toast.success(`Compared models successfully`)
    } else {
      toast.error('Comparison returned no results')
    }
...
const handleGenerateReport = async () => {
  ...
  try {
    ...
    if (report) {
      ...
      toast.error('Report generation returned no result')
    }
